...
|---|---|
| iron_sword | physical |
| rusted_sword | physical |
| war_axe | physical |
| goblin_dagger | physical |
| ice_club | ice |
| frost_cape | ice |
| mage_staff | physical |
| inferno_staff | fire |
...
| lava_boots | fire |
| harpy_bow | wind |
| drake_claw_dagger | water |
| hydra_venom_dagger | water |
| reaper_scythe | dark |
| cursed_blade | dark |
| star_blade | light |
| nova_blade | light |
...
Import `elementTable` dan `ELEMENT_EMOJI` dari `combat.ts` (sudah ada di file):
```ts
import { elementTable, ELEMENT_EMOJI } from '../../lib/rpg/combat';
```

Hitung weakness lalu tampilkan sebagai embed field terpisah, termasuk arrow ke elemen yang lemah:
```ts
// Weakness hint
...
> Catatan: `elementTable` perlu di-export dari `combat.ts` agar bisa dipakai di `hunt.ts`.

---

### 1.6 Data Monster (dungeon) *(BARU)*
**File:** `apps/bot/src/lib/rpg/dungeon/dungeon-data.ts`

#### 1.6.1 Tambah import dan type
...
Ubah definisi `DUNGEON_MONSTERS` — hapus `as const`, ganti dengan explicit type annotation agar field `element` bisa ditambahkan tanpa konflik:
```ts
...
#### 1.6.2 Fix `getMonster()` untuk boss

Bug: saat ini `getMonster` untuk boss hanya spread `boss` (`{ id, emoji }`) lalu ambil `base` dari DUNGEON_MONSTERS, tapi **tidak ambil `element`-nya**. Akibatnya boss selalu dapat fallback `'physical'`.
...
// Sesudahnya — spread baseData dulu biar element ikut terbawa:
export function getMonster(floor: number) {
  const boss = BOSSES[floor];
  if (boss) {
    const baseData = DUNGEON_MONSTERS.find((m) => m.id === boss.id);
...
#### 1.6.3 Isi element semua monster
...
### 1.7 Update dungeon-battle.ts *(BARU)*
**File:** `apps/bot/src/lib/rpg/dungeon/dungeon-battle.ts`

Tambah `ELEMENT_EMOJI` ke import yang sudah ada:
```ts
// Sebelumnya:
...
Ganti hardcode `element: 'physical'` di constructor `BattleEngine`:
```ts
// Sebelumnya:
element: 'physical',
...
Update call `buildBattleEmbed` di dalam fungsi `updateBattle` — tambah dua param baru:
```ts
const embed = buildBattleEmbed({
  ...
  monsterMaxHp,
  monsterElement: monster.element ?? 'physical',       // BARU
  monsterElementEmoji: ELEMENT_EMOJI[monster.element ?? 'physical'], // BARU
  playerName: username,
  playerHp: player.hp,
...
### 1.8 Update dungeon-ui.ts *(BARU)*
**File:** `apps/bot/src/lib/rpg/dungeon/dungeon-ui.ts`
...
Tambah dua param baru ke interface `buildBattleEmbed`:
```ts
export function buildBattleEmbed(params: {
  ...
  monsterHp: number;
  monsterMaxHp: number;
  monsterElement: Element;       // BARU
  monsterElementEmoji: string;   // BARU
  playerName: string;
  playerHp: number;
  playerMaxHp: number;
  lore: string;
...
Element ditampilkan di bawah HP bar monster — append ke `value` string field HP monster, **bukan field embed baru**:
```ts
// Sebelumnya:
{
...
{
  name: `${params.monsterEmoji} ${params.monsterName}`,
  value: `${ratioBar(Math.max(0, params.monsterHp), params.monsterMaxHp)} ${Math.max(0, params.monsterHp)}/${params.monsterMaxHp}\n${params.monsterElementEmoji} \`${params.monsterElement}\``,
  inline: true,
},
```
...
- Dungeon monster element juga punya fallback `monster.element ?? 'physical'`
...
- [x] Update UI `hunt.ts` tampilkan element + weakness hint
- [x] Test fire vs ice (1.5x)
- [x] Test physical default
- [x] Commit: `feat(elements): add 9-element system`
- [ ] Hapus `as const` di `DUNGEON_MONSTERS`, ganti explicit type annotation + tambah field `element`
- [ ] Isi element semua ~100 monster dungeon (slime×10, golem×10, specter×10, drake×10, warden×10, guardian×50)
- [ ] Fix `getMonster()` — boss spread `baseData` dulu biar `element` ikut terbawa
- [ ] Update `dungeon-battle.ts` — tambah `ELEMENT_EMOJI` import, ganti hardcode `element: 'physical'` → `monster.element ?? 'physical'`
- [ ] Update call `buildBattleEmbed` di `dungeon-battle.ts` — pass `monsterElement` + `monsterElementEmoji`
- [ ] Update `dungeon-ui.ts` — tambah param `monsterElement` + `monsterElementEmoji`, tampilkan di bawah HP bar monster
- [ ] Test `/dungeon` floor 2 blue_slime (water) + fire weapon → 1.5x
- [ ] Test `/dungeon` floor 3 red_slime (fire) + water weapon → 1.5x
- [ ] Test element muncul di bawah HP bar dungeon (non-boss & boss)
- [ ] Commit: `feat(elements): assign elements to dungeon monsters + dungeon UI`
...
git add apps/bot/src/lib/rpg/dungeon/dungeon-data.ts
git commit -m "feat(elements): assign elements to all dungeon monsters"

git add apps/bot/src/lib/rpg/dungeon/dungeon-battle.ts apps/bot/src/lib/rpg/dungeon/dungeon-ui.ts
git commit -m "feat(elements): show element in dungeon battle UI"
